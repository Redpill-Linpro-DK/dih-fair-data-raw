...
{
    public class ReceiveFullBatchBroadcast
    {
        private MessageHandlerHelper MessageHandlerHelper { get; set; }
        private const string queueName = "dataraw-receive-fullbatch-broadcast";

        public ReceiveFullBatchBroadcast(MessageHandlerHelper messageHandlerManager)
        {
            MessageHandlerHelper = messageHandlerManager;
        }

        [Singleton] // only allow 1 instance of this to run at any given time ... 
        [FunctionName("dataraw-receive-fullbatch-broadcast")]
